...
{
    GBytes *bytes = rc_alloc(size + sizeof(GBytes), NULL);
    if (!bytes)
        return NULL;
...
#define g_bytes_ref         ref
#define g_bytes_unref       deref

#define g_assert_cmpint(n1, cmp, n2)    do {        \
    gint64 __n1 = (n1), __n2 = (n2);                \
    if (__n1 cmp __n2) ; else                       \
    puts("ASSERT ERROR: " #n1 " " #cmp " " #n2);    \
} while (0)

#define g_test_init(argc, argv, p)      (void)0
#define g_test_run()                    (void)0

#define g_test_add_func(s, func)        do {        \
    printf("%s ......\n", s);                       \
    func();                                         \
    printf("%s ...... Done\n", s);                  \
} while (0)
...
segment_to_string (guint32 seq, guint32 ack, SegmentFlags flags)
{
  ...
  sprintf (out, "<SEQ=%u><ACK=%u><CTL=%s>", seq, ack, ctl);
  g_free (ctl);
...
create_sockets (Data *data, gboolean support_fin_ack)
{
  ...
  pseudo_tcp_socket_set_property(data->left, PROP_SUPPORT_FIN_ACK, &support_fin_ack);

  data->right = pseudo_tcp_socket_new(0, &cbs);
  pseudo_tcp_socket_set_property(data->right, PROP_SUPPORT_FIN_ACK, &support_fin_ack);
...
expect_socket_state (PseudoTcpSocket *socket, PseudoTcpState expected_state)
{
  ...
  pseudo_tcp_socket_get_property (socket, PROP_STATE, &state);
